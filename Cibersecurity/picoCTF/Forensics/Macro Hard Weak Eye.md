...
I use `binwalk ./Forensics\ is\ fun.pptm `
Then I watched this info:
...
```
size: 81, uncompressed size: 99, name: ppt/slideMasters/hidden
```

So I copy the the pptm file and changed the name to "copy.zip"

Then I unzip it and I was able to read the file going to 
...
I use slplit with python to remove the spaces 

```
flag = "Z m x h Z z o g c G l j b 0 N U R n t E M W R f d V 9 r b j B 3 X 3 B w d H N f c l 9 6 M X A 1 f Q"
flag = s.split(" ")
print("".join(flag))
```
...
Now I tried to decode it with base64 

`kali > echo "ZmxhZzogcGljb0NURntEMWRfdV9rbjB3X3BwdHNfcl96MXA1fQ" | base64 -d ; echo`

And that's all , I was able to see the flag
